...
jobRoute.post("/", async (req, res) => {
  ...
  try {
    ...
    if (description && designetion && experience && location && JobID && Jobtype && education && perksBenefits && candidateProfile && roleResponsibility && salary) {
      let FindId = await Jobsmodel.find({ JobID });
      if (FindId.length == 0) {
        let AllJob = await Jobsmodel.find();
        ...
        res.status(409).json({ message: "JobID exists" })
      }
