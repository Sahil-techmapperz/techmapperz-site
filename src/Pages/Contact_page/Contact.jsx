...
const Contact = () => {
  ...
  const handalesubmit = () => {
    ...
    if (name && email && mobile && projectType && projectdetails) {
      ...
      setHasError({ ...hasError, "email": hasError.email=true })
    }
      ...
      <section className={Styles.secend_section}>
        ...
        <div className={Styles.secend_section_secend}>
          <div className={Styles.Input_box}>
            ...
            { hasError.email? <label className={Styles.error_label}>you have to fill the email*</label>:null}
            </div>
            <div>
            ...
            <div>
              <label>Mobile*</label>
              <input className={`input ${hasError.mobile ? 'error' : ''}`} name='mobile' value={contactdata.mobile} onChange={handalechange} type={"number"} placeholder='enter mobile'></input>
            </div>
          </div>
          ...
          <div className={Styles.Submit_btn} onClick={handalesubmit}>
            <div></div>
            <h2>Submit</h2>
          </div>
        </div>
